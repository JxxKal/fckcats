...
async def saml_acs(
    request:  Request,
    pool:     asyncpg.Pool = Depends(get_pool),
    app_cfg:  Config       = Depends(app_config),
) -> RedirectResponse:
    try:
        from onelogin.saml2.auth import OneLogin_Saml2_Auth
    except ImportError:
    ...
    user = await _upsert_saml_user(
        pool, username, email, display_name, cfg.get("default_role", "user")
    )
    ...
    token = create_token(
        ...
        user["username"],
        user["role"],
        False,   # SAML-Konten haben kein lokales Passwort
    )
